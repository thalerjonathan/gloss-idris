...
-- a key in the `Map`.
data Graph = MkGraph (Map Vertex (Set Vertex))
...
fromEdges : StdGen -> List Edge -> Eff Scene [RND]
fromEdges gen es 
...
updatePosition 
        :  Double       -- Time since the last update
...
updatePositions : Double -> Scene -> Scene
...
inCircle : Point             -- Where the user has clicked
         -> Double             -- The scaling factor in the ViewPort
         -> Point             -- The position of the vertex
         -> Bool
inCircle p sca v 
  = magV (v - p) <= vertexRadius * sca

findVertex : Point -> Double -> Scene -> Maybe Vertex
